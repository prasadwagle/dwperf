-- PDW DML
--SELECT COALESCE(MAX(event_property_id), '0') AS last_value 
--create statistics event_id_stats on event_properties (event_id) with FULLSCAN
--FROM fact_events

-- 4 mins 28 seconds on 7 days (AND occurred_at > '07/24/2012')
-- 41 seconds on 1 day (AND occurred_at = '07/24/2012')

-- 6,176,339,442 rows processed in 4:46:32 without the IP address conversion

-- occurred at    AND occurred_at >='06/01/2012' AND occurred_at<'06/02/2012' -- 3 min 25 seconds

Create table fact_events_06012012
...
MAX(CASE property_name WHEN 'ip_address'  THEN property_value ELSE null END) AS ip_address
 ...
 WHERE (
   ...
   AND occurred_at >='06/01/2012' 
   AND occurred_at<'06/02/2012'
...
--       MAX(event_name)  AS event_name,
--       MAX(occurred_at) AS occurred_at,
--       MAX(CASE property_name WHEN 'network_id'  THEN property_value ELSE null END)::INT AS network_id,
--      MAX(CASE property_name WHEN 'user_id'     THEN property_value ELSE null END)::INT AS user_id,
--       MAX(CASE property_name WHEN 'session_id'  THEN SUBSTR(property_value,1,128) ELSE null END)      AS session_id,
--       MAX(CASE property_name WHEN 'client_id'   THEN property_value ELSE null END)::INT AS client_id,
--      MAX(CASE property_name WHEN 'controller'  THEN property_value ELSE null END)      AS controller,
--       MAX(CASE property_name WHEN 'action'      THEN property_value ELSE null END)      AS action,
--       MAX(CASE property_name WHEN 'id_param'    THEN SUBSTR(property_value,1,128) ELSE null END)      AS param_id,
--       MAX(CASE property_name WHEN 'user_agent'  THEN property_value ELSE null END)      AS user_agent,
--       MAX(CASE property_name WHEN 'http_method' THEN property_value ELSE null END)      AS http_method,
