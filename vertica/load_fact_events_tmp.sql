INSERT /*+direct*/ INTO fact_events_tmp
(
SELECT fe.*
FROM (
SELECT MAX(id) AS event_property_id,
 ...
 WHERE (
        (property_name IN ('network_id','user_id','client_id') AND REGEXP_LIKE(property_value, '^\d+\z'))
   ...
   AND id > 0
   AND occurred_at >= '2013-04-01' and occurred_at < '2013-04-02'
